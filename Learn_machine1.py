...
#
#     این الگوریتم‌ها فرض می‌کنند که داده‌ها از یک توزیع احتمال مشخص با پارامترهای معین پیروی می‌کنند.
#     هدف این الگوریتم‌ها این است که بهترین پارامترهای این توزیع فرضی را از داده‌های آموزشی یاد بگیرند.
#     مثال‌ها: رگرسیون خطی (فرض می‌کند رابطه خطی بین متغیرها وجود دارد و خطاها توزیع نرمال دارند)، مدل‌های بیزی (فرض توزیع برای داده‌ها و پارامترها).
#     مزیت: اگر فرضیات الگوریتم درست باشند، می‌توانند بسیار کارآمد باشند و به داده‌های کمتری نیاز داشته باشند.
#     عیب: اگر فرضیات الگوریتم درست نباشند، ممکن است عملکرد خوبی نداشته باشند.
#
# الگوریتم‌های غیرپارامتری:
#
#     این الگوریتم‌ها هیچ فرضی در مورد توزیع داده‌ها نمی‌کنند.
#     در عوض، آن‌ها مستقیماً از داده‌های آموزشی برای ساخت مدل استفاده می‌کنند.
#     مدل‌های غیرپارامتری معمولاً با افزایش حجم داده‌ها پیچیده‌تر می‌شوند.
#     مثال‌ها: الگوریتم KNN (که در مورد آن صحبت کردیم)، درخت تصمیم، ماشین بردار پشتیبان (SVM).
#     مزیت: انعطاف‌پذیر هستند و می‌توانند الگوهای پیچیده در داده‌ها را بدون نیاز به فرضیات توزیعی خاص یاد بگیرند.
#     عیب: ممکن است به داده‌های آموزشی بیشتری نیاز داشته باشند و محاسبات آن‌ها در برخی موارد سنگین‌تر باشد.
